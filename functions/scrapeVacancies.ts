...
const HH_HEADERS = {
  'User-Agent': 'JobRadar/1.0 (contact: jobradarsupport@example.com)',
  'HH-User-Agent': 'JobRadar/1.0 (contact: jobradarsupport@example.com)',
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (config.salary_from) {
      ...
      baseParams.currency = (config.salary_currency || 'RUR').replace('RUB', 'RUR');
